...
########
# modifications by me (<xylous.e@gmail.com>):
#   '-k'    option allows for specifying a hard disc image, which allows using
#           the filesystem more like on a VM; you can store things on hard disc
#           images
########
...
Options:
    ...
    -k [image]      attach an additional hard disc image
...
run_image() {
    ...
    if [[ -n "${hardimage}" ]]; then
        qemu_options+=(
            ...
            '-drive' "id=hd1,if=none,format=raw,file=${hardimage}"
        )
    fi
...
if (( ${#@} > 0 )); then
    while getopts 'abck:dhi:suv' flag; do
        case "$flag" in
            a)
                accessibility='on'
                ;;
